...
export default async function handler(req, res) {
  ...
  const { email, name = '', source = 'skills.aaxelera.com' } = req.body || {};
  ...
  const body = { email, first_name: name.split(' ')[0] || '', list_id: list, skip_if_in_workspace: true, custom_variables: { source, captured_at: new Date().toISOString() } };
  ...
  if (process.env.NOTIFY_WEBHOOK) { try { await fetch(process.env.NOTIFY_WEBHOOK, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ text: `New skills lead: ${email} ${name}` }) }); } catch (e) {} }
